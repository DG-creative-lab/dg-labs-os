import type { ApplicationCaseStudy, ApplicationClaim, EvolutionEntry } from './contracts';
...
export const validateApplicationProfile = ({
  ...
  for (const study of caseStudies) {
    ...
    if (study.roleSignals.length === 0 || study.roleSignals.some((signal) => !signal.trim())) {
      issues.push({
        scope: 'case-study',
        id: study.id,
        message: 'Case-study role signals must be non-empty.',
      });
    }
    for (const link of study.evidence) {
      if (!link.label.trim() || !isAbsoluteUrl(link.url)) {
        issues.push({
          scope: 'case-study',
          id: study.id,
          message: 'Case-study evidence links require labels and absolute HTTPS URLs.',
        });
      }
    }
  }
  ...
  for (const entry of evolution) {
    ...
    if (!entry.title.trim() || !entry.summary.trim()) {
      issues.push({
        scope: 'evolution',
        id: entry.title,
        message: 'Evolution title and summary are required.',
      });
    }
