...
export const stateMachineCatalog = [
  ...
  },
  {
    ...
    implementation: [
      ...
      'src/profiles/resume/runtime.ts',
      'src/profiles/resume/validation.ts',
    ],
    ...
    invariants: [
      ...
      'Resolving one profile never returns another profile module bundle, Network, Writing, or Resume projection.',
    ],
    nonDeterministicEffects: [],
  },
  {
    ...
    implementation: [
      ...
      'src/profiles/resume/viewModel.ts',
      'src/profiles/resume/markdown.ts',
    ],
    states: ['unresolved', 'resolved', 'rejected'],
    inputs: ['SELECT_PROFILE', 'SELECT_VARIANT', 'BUILD'],
    outputs: ['ResolvedProfileCv', 'ResumeViewModel', 'public CV assets', 'typed rejection'],
    invariants: [
      ...
      'The general CV is rendered from the selected profile Resume and approved evidence modules.',
      'Broken Resume references and drift between approved data and public Markdown fail the build.',
    ],
